...
- **Entrega**: **Fusão de F2-08 original + F4-05.** Efeitos, feats, classFeature, ancestry, heritage e equipamento equipado viram `EffectSource` com **todas** as regras. **Emenda de 2026-09-16 (após a entrega da ALQ-F4-19):** a garantia anterior — "os scanners antigos já foram migrados e não há mais caminho paralelo que conte em dobro" — **era falsa** e foi removida. A F4-19 unificou só a camada de _reconhecimento_ de regra no `RuleElementRegistry`; a de _execução/agregação_ continua nos scanners (`hp.ts`, `speed.ts` e `equipment.ts:243-244` seguem varrendo `doc.items` e escrevendo direto no derivado, sem passar por `RuleApplyContext`/`synthetics`). **O risco de contagem dupla é desta tarefa**: ou os call sites migram para o motor aqui, ou a materialização exclui explicitamente os seletores que os scanners já cobrem (`hp`, `speed`, `damage-dice-faces`). Motivo do adiamento na F4-19: mover a resolução de `@actor.level` para o handler zeraria o bônus de HP do Toughness, porque o avaliador de expressão em roll-time ainda não existe. `grantedConditions` do efeito viram EffectSource de condição no derivado, sem item órfão. Remover o efeito devolve os números.
...
- **Entrega**: Dono ou Mestre consome 1 unidade. Server valida ownership, decrementa `uses`/`quantity`, apaga com `autoDestroy`. Cura rolada no servidor e aplicada via `ActorMechanicsService.applyDamage` (alvos da foto `targetTokenIds`, ou o próprio ator). Efeitos de `effectRefs` copiados do pack com origem e início. Escrita atômica e um card. Hooks `onConsumed` registráveis (F5-11, F6-18, F7-07). **Emenda de 2026-09-16 (auditoria independente do pack `alchemical-items-core`, onda 2):** copiar o `Effect` para o ator não é suficiente sozinho — regra `kind:"strike"` dentro do efeito (ex.: garra/mandíbula de `Effect: Bestial Mutagen (Lesser)`, já publicado em `equipment-effects-core`) não tem conversor. O único código que materializa `strike` hoje é `stepCharInstinctStrikes` (satélite `systems/pf2e/src/derivations/instincts.ts:288`), e por design ele só processa item `type === "classFeature"` (Instintos do Bárbaro, issue #20) — um `Effect` nunca passa por ali. **Nenhuma tarefa deste plano é dona de generalizar esse conversor** — decidir isso (generalizar `stepCharInstinctStrikes` ou escrever caminho próprio) fica para quem pegar esta tarefa, ou se perde.
...
- **Entregue de fato (2026-09-16, onda 2)**: só a camada de **reconhecimento** foi unificada — `normalizeFlatModifierRule()` e o handler novo `item-alteration` passam pelo registro, mas `hp.ts`, `speed.ts` e `equipment.ts:243-244` continuam varrendo `doc.items` e agregando fora do motor. Os scanners **não** saíram do código, e o `apply()` do `item-alteration` é no-op documentado (não há slot em `RuleApplyContext`/`Synthetics` para mutar item). A resolução de `@actor.level` ficou no scanner de propósito: no handler genérico ela zeraria o bônus de HP do Toughness. Consequência registrada na ALQ-F2-08, que herda o risco de contagem dupla.
...
- **Entrega**: Physique (G): Bestial com bônus de Intimidação e garra/mandíbula; Juggernaut com resistência física ½ nível; Quicksilver como nota. Innervation: bônus de item G, telepatia/idiomas como nota. Respeita benefício suprimido. **Emenda de 2026-09-16 (auditoria independente do pack `alchemical-items-core`, onda 2):** a garra/mandíbula do Bestial depende de generalizar o conversor de `strike` (`stepCharInstinctStrikes`, satélite `systems/pf2e/src/derivations/instincts.ts:288`), que hoje só materializa `type === "classFeature"` (Instintos do Bárbaro) — `Effect` de mutágeno nunca passa por ali. **Nenhuma tarefa deste plano é dona dessa generalização** (mesma nota na ALQ-F2-11); sem ela, o bônus de Intimidação funciona (é `flat-modifier`), mas a garra/mandíbula em si não aparece.
