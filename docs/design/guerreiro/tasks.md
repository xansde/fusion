...
> Linha: **pessoal alfa**. Toda branch parte de `origin/feat/alquimista` nos dois repos (D-G16) e volta por PR de lote; satélite com PR no `xansde/fusion-systems-2e` + bump de pin no core.
...
escopo (D-15), com uma exceção: o **orçamento de reação** (D-G14), sem o qual a G-F4 não existe.

**Um aviso de calendário, não de escopo**: o plano do Alquimista tem **6 de 121** tarefas
integradas e o do Animista **0 de 40**. A G-F0 e a G-F1 deste plano — que já devolvem um
Guerreiro que mira, ataca, acerta pela regra e conta o próprio MAP — dependem de **duas** tarefas
externas (`ALQ-F1-04` e `ALQ-F1-05`, ambas da onda 3 do Alquimista). Da G-F2 em diante, a dependência cresce; a D-G18
traz o motor de reação para cá em vez de esperar a onda 16 daquele plano.

## 1. Decisões

### 1.1 Decisões propostas nesta rodada (2026-09-16)

Todas saem do inventário conferido contra o código. **São propostas minhas, com racional** — o
Alexandre confirma ou troca; a tarefa afetada está na última coluna e não começa antes.

| ID    | Tema                           | Proposta                                                                                                                                                                                                                                                                                                                                              | Racional                                                                                                                                                                                                                                                                                                                                                                                                       | Tarefas              |
| ----- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| D-G01 | Gesto de mira                  | **Clique com o botão direito num token alterna a mira**; `Esc` limpa a mira do usuário. Sem menu de contexto, sem botão na fila.                                                                                                                                                                                                                      | DEC-CBA-05 já decidiu que mirar é gesto espacial e vive no canvas, e o teste REQ-CBA-076 provou a remoção do controle do painel. Falta só o gesto. O botão esquerdo já é seleção e arraste.                                                                                                                                                                                                                    | GUE-F1-01            |
| D-G02 | Quem decide o crítico          | **O servidor**, sempre que houver alvo mirado: o card de golpe mostra o grau e oferece **um** botão de dano coerente com ele. Sem alvo, o card continua como hoje, com os dois botões.                                                                                                                                                                | A graduação já existe e é testada com socket (`chat-handler.ts:451-507`); e mesa sem mapa é caso real, não exceção.                                                                                                                                                                                                                                                                                            | GUE-F1-03            |
| D-G03 | MAP                            | **Contado pelo servidor** e mostrado na ficha: um botão de golpe que já traz o MAP corrente. Os três botões viram um seletor recolhido de "forçar MAP", para o ataque que não conta.                                                                                                                                                                  | O jogador contar sozinho é a fonte nº 1 de erro de mesa, e o ponto de reset (`turnStart`) já existe e é testado.                                                                                                                                                                                                                                                                                               | GUE-F1-04, GUE-F1-05 |
...
| D-G06 | Flanqueio                      | **Automático**: o servidor calcula a partir das posições e aplica desprevenido como estado derivado, não como condição escrita à mão, com chave de desligar por mundo.                                                                                                                                                                                | É a condição mais citada da classe (16 vezes nos textos) e a geometria já existe e é testada em `shared/grid` e `shared/vision` — falta a regra consumir.                                                                                                                                                                                                                                                      | GUE-F3-02            |
...
| D-G13 | Ataque em vários alvos         | **Rever a REQ-ACH-074** para permitir N ataques correlacionados num card só, em vez de N mensagens soltas.                                                                                                                                                                                                                                            | Hoje a API diz, por escrito, que um ataque carrega exatamente um alvo; 10 documentos do Guerreiro não cabem nisso.                                                                                                                                                                                                                                                                                             | GUE-F6-04            |
| D-G14 | Orçamento de reação            | **Emenda à D-15 no recorte da reação**: o Guerreiro acrescenta **uma reação por rodada**, com reset no início do turno. As três ações do turno continuam fora de escopo.                                                                                                                                                                              | A `ALQ-F7-05` decidiu, por escrito, que "aceitar duas reações no mesmo turno é permitido (não há orçamento)" — o que serve para reação de 1/dia e quebra a classe inteira aqui: Reflexos Táticos e Represálias Ilimitadas só fazem sentido contra um limite base.                                                                                                                                              | GUE-F4-01            |
...
| D-G16 | Base das branches              | `feat/guerreiro` nasce de **`feat/alquimista`** nos dois repos, não de `alfa/app`.                                                                                                                                                                                                                                                                    | Metade do plano depende de peças que só existem lá (aplicar dano, condição, TurnHooks, registro de rule elements, REQ-PF2-207). Nascer de `alfa/app` significaria reimplementar ou ficar bloqueado desde a onda 1.                                                                                                                                                                                             | —                    |
...
| D-G18 | Quem entrega o motor de reação | **Como está escrito**: a G-F4 **consome** o `ReactionTrigger` da `ALQ-F7-05` e entrega só o que ele deliberadamente não tem (o teto de 1/rodada, os gatilhos marciais, a anotação no canto). **A proposta é antecipar**: mover o conteúdo da `ALQ-F7-05` para a `GUE-F4-02`, e o Alquimista passa a consumir.                                         | A `ALQ-F7-05` está na **onda 16 de 19** do plano do Alquimista, que tem 6 de 121 tarefas integradas. O Guerreiro tem duas habilidades de **nível 1** que são reação e 23 documentos dependentes: é a classe que justifica o motor, e esperar a onda 16 de outra frente deixa a G-F4 parada. Se o Alexandre mantiver como está, nada no plano muda — as fichas já estão escritas nesse caminho; só a data anda. | GUE-F4-01, GUE-F4-02 |
...
| D-15                     | Alquimista | Contador de ações por turno fora de escopo                                                                                           | 16 documentos do Guerreiro seguem sem fechar (§8); a D-G14 abre a exceção da reação |
...
| `ReactionEconomy`                   | GUE-F4-01 | GUE-F4-02..04, GUE-F5-02                   |
...
### 2.4 `ReactionEconomy` e `ReactionOffer` (GUE-F4-01, GUE-F4-02)

```ts
// packages/shared/src/combat/reaction.ts
interface ReactionEconomy {
  combatantId: string;
  available: boolean;
  spentOn?: string;
}
...
interface ReactionOffer {
  id: string; // idempotência: responder duas vezes não gasta duas reações
  ...
  /** Reações elegíveis do ator, já filtradas pelo servidor. */
  options: { itemSourceId: string; label: string }[];
  /** Fecha sozinha quando a ação que disparou termina de resolver. */
  expiresAt: number;
}
```

A oferta **não bloqueia** o fluxo de ninguém: ela chega como anotação no canto do jogador dono
(mesmo slot da D-16 do Alquimista) e some ao expirar. Quem não respondeu, não reagiu — é a
mesma regra da mesa.
...
- **Três frentes sobre a mesma base.** Medido tarefa a tarefa nos dois planos anteriores, os
  arquivos disputados que este plano também toca: `chat-handler.ts` (4 tarefas do Alquimista, 0
  do Animista — e é onde o Guerreiro mais mexe), `strikes.ts` (4 do Alquimista), `derive-runner.ts`
  (3), `effectsEngine.ts` (3 + 1), `characterSheetVM.ts` (7 + 3), `CharacterSheet.svelte` (7 + 2),
  `planVM.ts` (3 + 4), `systems/pf2e/src/index.ts` (6), `combatStore.svelte.ts` (1 — a `ALQ-F1-05`).
...
> **Fronteira revisada (2026-09-16)**: a primeira leitura desta rodada dizia "não existe nada de reação" — impreciso num ponto decisivo. O plano do Alquimista já tem tarefa dona do motor genérico de gatilho e oferta: **`ALQ-F7-05` — "ReactionTrigger: gatilhos e oferta de reação"** (satélite, `systems/pf2e/src/actions/reaction-trigger.ts`, sonnet/high, onda 16 do plano do Alquimista). Ela entrega `ReactionOfferDefinition`, avaliação após o evento (`TurnHooks`, `onDamageApplied`, `onRollResolved`), card sussurrado (`redaction.ts`) e as ações `reaction:accept`/`reaction:decline`, validando aceite **só por permissão e frequência declarada — sem orçamento** (o próprio teste dela prova que aceitar duas reações no mesmo turno é permitido hoje). D-18 (Alquimista, herdada): a oferta **não tem cronômetro** — some no próximo evento do mesmo personagem, nunca um prazo em segundos. As quatro fichas abaixo foram reescritas para nunca reimplementar esse motor: usam-no, citam-no como dependência, e entregam só o que ele deliberadamente não cobre (o teto de 1 reação/rodada que D-G14 pede, os gatilhos do eixo marcial, e a ligação com a anotação de canto).

### GUE-F4-01 — Economia de reação: uma por rodada, reset no turno (D-G14)

- **Repo**: ambos
- **Onde**: novo `packages/server/src/combat/reaction-budget-store.ts` (core — mesmo padrão de `packages/server/src/combat/targeting-store.ts`: Map em memória por combatente, nunca persistido) + uma função de registro, no mesmo arquivo novo, que chama `eventBus.onLifecycle("turnStart", ...)` — a mesma API pública que `packages/server/src/combat/target-handler.ts:169-180` já usa para `"turnEnd"` (o bus, `combat-event-bus.ts`, não é alterado, só ganha um segundo consumidor real ao lado do que a GUE-F1-04 registra para o MAP). Satélite: `external/fusion-systems-2e/systems/pf2e/src/actions/reaction-trigger.ts` — arquivo real entregue pela `ALQ-F7-05`, herdado porque `feat/guerreiro` nasce de `feat/alquimista` (D-G16) — ganha uma chamada nova a este store do core, antes de confirmar `reaction:accept`.
- **Entrega**: A `ALQ-F7-05` já entrega o motor de oferta/aceite inteiro, deliberadamente **sem orçamento**. Isso é gap real para o Guerreiro: 12 talentos de reação mais Golpe Reativo sem teto por rodada equivalem a reações ilimitadas, o que D-G14 assume como exceção explícita ao D-15 ("sem isso não há reação nenhuma"). Esta tarefa entrega só o que falta — um `ReactionEconomy` por combatente (`available: boolean`), resetado no início do turno de quem é dono do combatente pelo mesmo bus que já zera outras coisas de rodada, e uma checagem a mais dentro do `reaction:accept` herdado: se `available` for falso, recusa antes de chegar à validação de permissão/frequência da `ALQ-F7-05`. Nenhuma peça do motor de oferta é reescrita — nem o card sussurrado, nem a checagem de frequência, nem `reaction:decline`.
- **Depende de**: GUE-F0-01; **ALQ-F7-05** (contrato de oferta/aceite que este orçamento complementa), **ALQ-F1-04** (TurnHooks)
- **Paralelo com**: GUE-F0-07, GUE-F1-02, GUE-F1-05
- **Modelo / esforço**: sonnet / high — estado autoritativo de combate com anti-cheat, tocando um arquivo que a Alquimista entrega primeiro.
- **Teste (TDD)**: `reaction-budget.test.ts` (helper de porta, socket real): combatente com reação disponível aceita a primeira oferta da rodada; uma segunda oferta na MESMA rodada é recusada por este gate mesmo passando na checagem de permissão+frequência da `ALQ-F7-05` (mesmo cenário do teste dela — "aceitar duas reações no mesmo turno é permitido" — mas com o gate desta tarefa ativo, provando que o teto agora existe por cima); `turnStart` do combatente devolve `available:true`; o turno de outro combatente não mexe no orçamento de quem ainda não jogou; um `reaction:accept` forjado para um combatente que não é o dono é recusado (mesmo padrão de anti-cheat do `combat-own-turn.test.ts`, G054).
- **Prova visual (print)**: coberta por GUE-F4-03/GUE-F4-04 — o orçamento em si não tem tela própria, aparece como a oferta desaparecendo depois de aceita ou como a recusa da segunda oferta.
...
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **O plano depende de uma frente que mal começou** | O Alquimista tem 6 de 121 tarefas integradas e o Animista 0 de 40. Metade da G-F2 e toda a G-F5 esperam peças que ainda não existem. | A G-F0 e a G-F1 não dependem de nada do Alquimista além da `ALQ-F1-05` (onda 3 dele); a D-G18 antecipa o motor de reação para cá em vez de esperar a onda 16 dele. Tarefa que espera fica `bloqueada` no estado e a rodada seguinte tenta de novo — nunca se reimplementa a peça do outro plano. |
| `chat-handler.ts` é o gargalo das três frentes    | conflito no merge dos PRs de lote                                                                                                    | O Guerreiro entra por `checkContext` novo (união discriminada aberta) e por um módulo próprio (`map-counter.ts`), não editando o corpo da graduação; a `ALQ-F5-10` e a `ALQ-F4-09` mexem no mesmo trecho e precisam entrar antes ou depois, nunca na mesma onda.                                 |
...
| Flanqueio automático erra                         | desprevenido aplicado onde a mesa discorda                                                                                           | É estado derivado e visível (badge no token), não condição escrita; o Mestre desliga por mundo.                                                                                                                                                                                                  |
