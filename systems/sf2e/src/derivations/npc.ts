...
export const stepNpcPerception: DeriveStep = {
  ...
  run(doc, ctx) {
    ...
    // tools/importer-pf2e/src/transform.mjs normalizeActorSystem, shared by
    // both pf2e and sf2e importers) store perception at
    // `system.perception.mod`, not `system.attributes.perception.mod`.
    // Reading only the nested path silently produced base=0 for every
    // imported NPC (e.g. Akata, Perception +6, rolled initiative at +0) —
    // proved empirically against systems/sf2e/packs/bestiary-core/documents.json.
