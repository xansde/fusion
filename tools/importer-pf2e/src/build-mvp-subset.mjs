...
/**
 * Pack slugs published for the GM alone (`audience: "gm"` — REQ-CPD-072,
 * REQ-PF2-141/142). Creature packs are here; hazard packs (Actor subtype
 * `hazard`) are NOT generated by this pipeline today, so REQ-PF2-142 is
 * prospective — when the first hazard pack is generated, its slug goes in this
 * set instead of being decided in that pack's manifest.
 *
 ...
 * (REQ-CMP-010a, REQ-CPD-071/074).
 */
const GM_ONLY_PACK_SLUGS = new Set(["bestiary-core"]);
...
function writePack(slug, docs, manifest, packsOutDir = PACKS_OUT_DIR) {
  ...
  // for every pack — never decided case by case at generation time — so that
  // regenerating the bestiary can no longer silently reopen it to the players.
  const finalManifest = {
    ...manifest,
    audience: manifest.audience ?? (GM_ONLY_PACK_SLUGS.has(slug) ? "gm" : "all"),
