...
**Racional:** DEC-TOK-06 (`specs/41-token.md`) apagou o conceito de "token controlado pelo usuário" do projeto inteiro — ownership passou a ser sempre sobre o Actor, nunca sobre uma peça. A `09` era a última consumidora da noção antiga. Ancorar o bubble no Actor, não na peça, mantém o comportamento coerente em cena sem nenhuma peça do personagem (não aparece) e em cena com mais de uma (aparece em todas), sem reintroduzir uma segunda noção de posse.
