...
4. Tela "Feats" é inútil (Plano cumpre o papel) — follow-up de remoção (NÃO remover sem OK explícito). Falta tela de Pets → deixar para futuro, MAS rodar agente estudando regras de pets/companheiros/familiares e preparar specs.
...
### Fase 3
- **B3 — auditoria item a item da ficha do Tobias** (opus, com playwright + cópia do mundo). Para CADA seleção (ancestralidade Ratfolk, herança, background, boosts, perícias, Starlit Span, Ratspeak, Tinkering Fingers?, Magus's Analysis, Bon Mot, Alchemist Dedication, Steel Your Resolve, feats nível 3, skill increase, features automáticas): regras esperadas (doc do pack: descrição+REs) × o que a ficha mostra (chips/grants/ações/magias) × tradução em TODAS as superfícies × rolagens clicáveis. Output: tabela de gaps priorizada. Fixes residuais em seguida (agentes por território conforme o achado).
...
- Remoção/repropósito da aba Feats da ficha (Plano cobre) — aguarda OK explícito.
