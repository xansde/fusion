...
describe("confirm()", () => {
  ...
  it("uses default labels when none provided", () => {
    ...
    const d = pendingDialogs[0] as { confirmLabel: string; cancelLabel: string; resolve: (v: boolean) => void };
  ...
  it("carries the message string", () => {
    ...
    expect((pendingDialogs[0] as { message: string; resolve: (v: boolean) => void }).message).toBe("Are you really sure?");
    (pendingDialogs[0] as { resolve: (v: boolean) => void }).resolve(false);
  });
});
...
describe("prompt()", () => {
  ...
  it("carries the defaultValue", () => {
    ...
    expect((pendingDialogs[0] as { defaultValue: string; resolve: (v: string|null) => void }).defaultValue).toBe("default text");
    (pendingDialogs[0] as { resolve: (v: string|null) => void }).resolve(null);
  });

  it("empty defaultValue when not provided", () => {
    prompt("Enter:").catch(() => {});
    expect((pendingDialogs[0] as { defaultValue: string; resolve: (v: string|null) => void }).defaultValue).toBe("");
    (pendingDialogs[0] as { resolve: (v: string|null) => void }).resolve(null);
  });
});
